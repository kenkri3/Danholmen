...
const KEYS = {
  ...
  websiteImages: "danholmen_website_images",
} as const;
...
export function initStore(): void {
  ...
  if (!localStorage.getItem(KEYS.websiteImages))
    setItem(KEYS.websiteImages, getDefaultWebsiteImages());
}

function getDefaultWebsiteImages(): WebsiteImages {
  return {
    heroImage: "/hero-sauna.jpg",
    heroOverlayOpacity: 0.55,
    saunaCards: {
      "sauna-1": "/sauna-araas.jpg",
      "sauna-2": "/sauna-ormelet.jpg",
      "sauna-3": "/sauna-medo.jpg",
    },
    aboutSectionImage: "/hero-sauna.jpg",
    galleryImages: ["/sauna-araas.jpg", "/sauna-ormelet.jpg", "/sauna-medo.jpg"],
    updatedAt: new Date().toISOString(),
    heroVideoSeason: "summer",
  };
}
...
export function refundBooking(id: string, amount?: number, reason?: string): Booking | null {
  const items = getBookings();
  const idx = items.findIndex((b) => b.id === id);
  if (idx >= 0 && items[idx].status !== "refunded") {
    items[idx].status = "refunded" as BookingStatus;
    items[idx].refundedAt = new Date().toISOString();
    items[idx].refundAmount = amount ?? items[idx].totalPrice;
    items[idx].refundReason = reason ?? null;
    items[idx].paymentStatus = "free";
    setItem(KEYS.bookings, items);
    return items[idx];
  }
  return null;
...
// ------------------------------------------------------------------
// Membership Tier Config (for landing page)
// ------------------------------------------------------------------
export function getMembershipTiers(): MembershipTierConfig[] {
  const config = getSystemConfig();
  return (config.membershipTiers ?? []).sort((a, b) => a.order - b.order);
}

export function getActiveMembershipTiers(): MembershipTierConfig[] {
  return getMembershipTiers().filter((t) => t.isActive).sort((a, b) => a.order - b.order);
}

export function saveMembershipTier(tier: MembershipTierConfig): void {
  const config = getSystemConfig();
  ...
  saveSystemConfig({ ...config, membershipTiers: tiers });
}

export function getAllMembershipTiers(): MembershipTierConfig[] {
  const config = getSystemConfig();
  return config.membershipTiers ?? [];
}

export function updateMembershipTier(updated: MembershipTierConfig): void {
  const config = getSystemConfig();
  const tiers = config.membershipTiers ?? [];
...
export function toggleMembershipTierActive(tierId: string): MembershipTierConfig | null {
  const config = getSystemConfig();
  const tiers = config.membershipTiers ?? [];
  const idx = tiers.findIndex((t) => t.id === tierId);
  if (idx < 0) return null;
  tiers[idx] = { ...tiers[idx], isActive: !tiers[idx].isActive };
  saveSystemConfig({ ...config, membershipTiers: tiers });
  return tiers[idx];
}

export function deleteMembershipTier(tierId: string): void {
  const config = getSystemConfig();
  const tiers = (config.membershipTiers ?? []).filter((t) => t.id !== tierId);
  saveSystemConfig({ ...config, membershipTiers: tiers });
}

export function reorderMembershipTiers(orderedIds: string[]): void {
  const config = getSystemConfig();
  const tiers = config.membershipTiers ?? [];
  const ordered = orderedIds.map((id) => tiers.find((t) => t.id === id)).filter(Boolean) as MembershipTierConfig[];
  const rest = tiers.filter((t) => !orderedIds.includes(t.id));
  saveSystemConfig({ ...config, membershipTiers: [...ordered, ...rest] });
}
...
const CUSTOMER_KEY = "danholmen_saved_customer";

export function getSavedCustomer(): SavedCustomerInfo | null {
  return getItem(CUSTOMER_KEY, null);
}

export function saveCustomer(info: SavedCustomerInfo): void {
  setItem(CUSTOMER_KEY, info);
}
...
// ------------------------------------------------------------------
// Website Images (for landing page easy image swapping)
// ------------------------------------------------------------------
export function getWebsiteImages(): WebsiteImages {
  return getItem(KEYS.websiteImages, getDefaultWebsiteImages());
}

export function updateWebsiteImages(updates: Partial<WebsiteImages>): WebsiteImages {
  const current = getWebsiteImages();
  const next: WebsiteImages = { ...current, ...updates, updatedAt: new Date().toISOString() };
  setItem(KEYS.websiteImages, next);
  return next;
}

export function updateHeroImage(image: string): WebsiteImages {
  return updateWebsiteImages({ heroImage: image });
}

export function updateHeroVideoSeason(season: Season): WebsiteImages {
  return updateWebsiteImages({ heroVideoSeason: season });
}

export function updateSaunaCardImage(saunaId: string, image: string): WebsiteImages {
  const current = getWebsiteImages();
  const saunaCards = { ...current.saunaCards, [saunaId]: image };
  return updateWebsiteImages({ saunaCards });
}

export function updateAboutImage(image: string): WebsiteImages {
  return updateWebsiteImages({ aboutSectionImage: image });
}

export function updateGalleryImages(images: string[]): WebsiteImages {
  return updateWebsiteImages({ galleryImages: images });
}

export function updateHeroOverlayOpacity(opacity: number): WebsiteImages {
  return updateWebsiteImages({ heroOverlayOpacity: opacity });
}

export function resetWebsiteImages(): WebsiteImages {
  const defaults = getDefaultWebsiteImages();
  setItem(KEYS.websiteImages, defaults);
  return defaults;
}
